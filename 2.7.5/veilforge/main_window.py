...
HELP_TEXT = (
    ...
    "\n"
    "TOKENS\n"
    "- Import Token: add one or multiple PNG/JPG tokens on the current map\n"
    ...
    "- Imported token size is normalized automatically (adjust via Parameters)\n"
...
class MainWindow(QMainWindow):
    ...
    def _resolve_session_media_path(self, saved_path: str, session_json_path: Path) -> Path | None:
        ...
        filter_text = (
            "Map/Media/Session files (*.png *.jpg *.jpeg *.webp *.bmp *.gif *.pdf *.json "
        ...
        located, _ = QFileDialog.getOpenFileName(
            ...
            str(self.default_videos_dir if self._is_video_path(raw) else self.default_maps_dir),
            filter_text,
        )
        if not located:
            return None
    ...
    def _default_sessions_dir(self) -> Path:
        """Default sessions folder (configurable from Parameters)."""
        d = getattr(self, "default_sessions_dir", None)
        if d is None:
    ...
    def _fallback_browser_dir(self) -> Path:
        d = Path(r"D:/_JDR_Ressources/Veilforge 2.7.5")
        if d.exists():
            return d.resolve()
        return self._app_dir().resolve()

    def _sanitize_existing_dir(self, raw: str | None, fallback: Path) -> Path:
        try:
    ...
    def _sanitize_or_create_dir(self, raw: str | None, fallback: Path) -> Path:
        try:
            txt = str(raw or "").strip()
            p = Path(txt).expanduser() if txt else fallback
    ...
    def _is_video_path(self, path: str | Path) -> bool:
        ext = Path(path).suffix.lower()
        return ext in {".mp4", ".webm", ".m4a", ".mkv", ".mov", ".avi", ".flv", ".m2ts", ".ts", ".3gp"}
    ...
    def __init__(self):
        ...
        fallback_dir = self._fallback_browser_dir()
        self.default_maps_dir = self._sanitize_existing_dir(self.settings.value("default_maps_dir", str(fallback_dir)), fallback_dir)
        self.default_videos_dir = self._sanitize_existing_dir(self.settings.value("default_videos_dir", str(self.default_maps_dir)), self.default_maps_dir)
        self.default_tokens_dir = self._sanitize_existing_dir(self.settings.value("default_tokens_dir", str(self.default_maps_dir)), self.default_maps_dir)
        default_sessions_fallback = self._app_dir() / "data" / "sessions"
        self.default_sessions_dir = self._sanitize_or_create_dir(
            self.settings.value("default_sessions_dir", str(default_sessions_fallback)),
            default_sessions_fallback,
        )
        self.grid_default_on_enable = str(self.settings.value("grid_default_on_enable", "Square") or "Square")
        if self.grid_default_on_enable not in ("None", "Square", "Hex"):
            self.grid_default_on_enable = "Square"
        self.token_import_scale = max(0.4, min(3.0, float(self.settings.value("token_import_scale", 1.0, type=float))))
        self.token_batch_spacing = max(40.0, min(320.0, float(self.settings.value("token_batch_spacing", 96.0, type=float))))
        self.last_open_media_kind = str(self.settings.value("last_open_media_kind", "map") or "map")
        ...
        self._pending_video_tokens: list[dict] = []
        self._pending_video_drawings: list[Stroke] = []

        # Windows
        self.canvas = DMCanvas()
        self.canvas.token_import_scale = float(self.token_import_scale)
        ...
        self.btn_parameters.setProperty("vfRole", "secondary")
        ...
        # Main content: left tools column + right canvas.
        content_row = QHBoxLayout()
        content_row.setSpacing(6)
        v.addLayout(content_row, 1)

        self.left_tools = QFrame()
        self.left_tools.setObjectName("vfLeftTools")
        self.left_tools.setFrameShape(QFrame.Shape.StyledPanel)
        self.left_tools.setMinimumWidth(220)
        self.left_tools.setMaximumWidth(290)
        left_lay = QVBoxLayout(self.left_tools)
        left_lay.setContentsMargins(8, 8, 8, 8)
        left_lay.setSpacing(10)
        content_row.addWidget(self.left_tools, 0)

        sec_fog = QLabel("Fog")
        sec_fog.setProperty("vfSectionTitle", "true")
        left_lay.addWidget(sec_fog)

        row_fog = QVBoxLayout()
        row_fog.setSpacing(4)
        left_lay.addLayout(row_fog)
        ...
        row_fog_top = QHBoxLayout()
        row_fog_top.setSpacing(4)
        row_fog_top.setContentsMargins(0, 0, 0, 0)
        row_fog_top.addWidget(self.btn_undo)
        row_fog_top.addWidget(self.btn_redo)
        row_fog.addLayout(row_fog_top)
        row_fog.addWidget(self.btn_reset)
        row_fog.addWidget(self.btn_fog)

        sep_fog = QFrame()
        sep_fog.setObjectName("vfSectionSeparator")
        sep_fog.setFrameShape(QFrame.Shape.HLine)
        sep_fog.setFrameShadow(QFrame.Shadow.Sunken)
        left_lay.addWidget(sep_fog)

        sec_grid = QLabel("Grid && Calibrate")
        sec_grid.setProperty("vfSectionTitle", "true")
        left_lay.addWidget(sec_grid)
        ...
        self.slider_grid_cell = QSlider(Qt.Orientation.Horizontal)
        self.slider_grid_cell.setRange(5, 500)
        self.slider_grid_cell.setValue(self.spin_grid.value())
        self.slider_grid_cell.setToolTip("Adjust grid cell size with the mouse")
        ...
        row4_type = QHBoxLayout()
        row4_type.addWidget(QLabel("Type"))
        row4_type.addWidget(self.cmb_grid)
        row4.addLayout(row4_type)

        row4_cell = QHBoxLayout()
        row4_cell.addWidget(QLabel("Grid cell size (px on map)"))
        row4_cell.addWidget(self.spin_grid, 1)
        row4.addLayout(row4_cell)
        row4.addWidget(self.slider_grid_cell)

        row4_alpha = QHBoxLayout()
        row4_alpha.addWidget(QLabel("Grid alpha"))
        row4_alpha.addWidget(self.grid_alpha)
        row4.addLayout(row4_alpha)

        row4.addWidget(self.chk_grid_player)
        row4.addWidget(self.btn_grid_cal)

        sep_grid = QFrame()
        sep_grid.setObjectName("vfSectionSeparator")
        sep_grid.setFrameShape(QFrame.Shape.HLine)
        sep_grid.setFrameShadow(QFrame.Shadow.Sunken)
        left_lay.addWidget(sep_grid)

        sec_annot = QLabel("Annotate")
        sec_annot.setProperty("vfSectionTitle", "true")
        left_lay.addWidget(sec_annot)

        row4b = QVBoxLayout()
        row4b.setSpacing(4)
        left_lay.addLayout(row4b)
        ...
        row4b.addWidget(self.chk_annotate)
        row4b.addWidget(self.chk_snap)

        row4b_color = QHBoxLayout()
        row4b_color.addWidget(QLabel("Color"))
        row4b_color.addWidget(self.btn_color)
        row4b.addLayout(row4b_color)

        row4b_width = QHBoxLayout()
        row4b_width.addWidget(QLabel("Width"))
        row4b_width.addWidget(self.spin_draw_w)
        row4b.addLayout(row4b_width)

        row4b_style = QHBoxLayout()
        row4b_style.addWidget(QLabel("Style"))
        row4b_style.addWidget(self.cmb_dash)
        row4b.addLayout(row4b_style)

        row4b_alpha = QHBoxLayout()
        row4b_alpha.addWidget(QLabel("Alpha"))
        row4b_alpha.addWidget(self.slider_draw_alpha)
        row4b.addLayout(row4b_alpha)
        ...
        row4b.addWidget(self.btn_del_anno)
        row4b.addStretch(1)

        sep_annot = QFrame()
        sep_annot.setObjectName("vfSectionSeparator")
        sep_annot.setFrameShape(QFrame.Shape.HLine)
        sep_annot.setFrameShadow(QFrame.Shadow.Sunken)
        left_lay.addWidget(sep_annot)

        sec_player = QLabel("Player screen")
        sec_player.setProperty("vfSectionTitle", "true")
        left_lay.addWidget(sec_player)
        ...
        self.btn_calib_display.setToolTip(f'Current calibration: ppi : {self.px_per_inch_real:.2f}')
        ...
        for _btn in (self.btn_pl_left, self.btn_pl_right, self.btn_pl_up, self.btn_pl_down):
        ...
        row5_pan = QHBoxLayout()
        row5_pan.setSpacing(2)
        row5_pan.setContentsMargins(0, 0, 0, 0)

        row5_pan.addStretch(1)
        row5_pan.addWidget(self.btn_pl_left, alignment=Qt.AlignmentFlag.AlignCenter)
        row5_pan_ud = QWidget()
        row5_pan_ud_lay = QVBoxLayout(row5_pan_ud)
        row5_pan_ud_lay.setSpacing(2)
        row5_pan_ud_lay.setContentsMargins(0, 0, 0, 0)
        row5_pan_ud_lay.addWidget(self.btn_pl_up)
        row5_pan_ud_lay.addWidget(self.btn_pl_down)
        row5_pan.addWidget(row5_pan_ud, alignment=Qt.AlignmentFlag.AlignCenter)
        row5_pan.addWidget(self.btn_pl_right, alignment=Qt.AlignmentFlag.AlignCenter)
        row5_pan.addStretch(1)

        row5.addLayout(row5_pan)

        row5_center = QHBoxLayout()
        row5_center.addStretch(1)
        row5_center.addWidget(self.btn_pl_center)
        row5_center.addWidget(self.btn_calib_display)
        row5_center.addStretch(1)
        row5.addLayout(row5_center)

        sep_player = QFrame()
        sep_player.setObjectName("vfSectionSeparator")
        sep_player.setFrameShape(QFrame.Shape.HLine)
        sep_player.setFrameShadow(QFrame.Shadow.Sunken)
        left_lay.addWidget(sep_player)

        sec_params = QLabel("Parameters")
        sec_params.setProperty("vfSectionTitle", "true")
        left_lay.addWidget(sec_params)

        row6 = QHBoxLayout()
        row6.setSpacing(6)
        left_lay.addLayout(row6)
        row6.addWidget(self.btn_parameters)
        row6.addStretch(1)
        ...
        content_row.addWidget(self.canvas_wrap, 1)
        ...
        self.cta_open.setProperty("vfRole", "secondary")
        ...
        self.btn_parameters.clicked.connect(self.show_parameters_dialog)
        ...
        self.spin_grid.valueChanged.connect(self.slider_grid_cell.setValue)
        self.slider_grid_cell.valueChanged.connect(self.spin_grid.setValue)
...
QDialog QLineEdit, QMessageBox QLineEdit, QDialog QTextEdit, QMessageBox QTextEdit {
    background: #12212d;
    color: #eaf2f9;
...
QFrame#vfSectionSeparator {
    border: none;
    border-top: 1px solid #2d4558;
    min-height: 1px;
    max-height: 1px;
}
...
"""
    ...
    def _on_dm_video_frame(self, frame):
        ...
        # First valid frame initializes map-space reference for token/fog tools.
        if self.current_video_frame_img is not None and getattr(self.canvas, "_map", None) is None:
            try:
                ref = self.current_video_frame_img.copy()
                self.map_img = ref
                self.canvas.set_images(self.map_img, None, drawings=self._pending_video_drawings or [])
                self.canvas.reset_fog()
                if self._pending_video_tokens:
                    self.canvas.tokens = self._pending_video_tokens
                    self.canvas.selected_token_index = -1
                    self.canvas.tokensChanged.emit()
                    self.canvas.update()
                    self._pending_video_tokens = []
                self._pending_video_drawings = []
                self.canvas.set_video_mode(True)
                # Keep player map-space in sync so tokens align on video mode.
        ...
        try:
            self.canvas.set_video_frame(self.current_video_frame_img)
        except Exception:
            pass
    ...
    def _set_video_mode(self, enabled: bool, path: str | None = None) -> None:
        ...
        """
        if enabled and path:
            # Keep DM canvas active so token interactions work on video maps.
            self.canvas_stack.setCurrentWidget(self.canvas)
            self._stop_media()
            try:
            ...
            try:
                ...
                self.canvas.set_video_frame(None)
            except Exception:
                pass
            self._sync_video_grid_overlay()
    ...
    def _load_session_path(self, path: str) -> None:
        ...
        if self.loaded.is_video:
            self._pending_video_tokens = self._deserialize_tokens(getattr(data, "tokens", []))
            self._pending_video_drawings = [Stroke.from_dict(d) for d in data.drawings]
            self.map_img = None
    ...
    def show_help(self):
        ...
        credits_box.setMarkdown(
            ...
            "\n"
            "**AI co-dev (versions 2.7.x):** Cedric Dagobert\n"
            "\n"
    ...
    def show_parameters_dialog(self):
        ...
        # Token import scale
        lbl_scale = QLabel("Token import scale")
        slider_scale = QSlider(Qt.Orientation.Horizontal)
        slider_scale.setRange(40, 300)
        slider_scale.setValue(int(round(float(self.token_import_scale) * 100.0)))
        lbl_scale_val = QLabel("")

        # Match other styled dialogs: white parameter labels.
        label_style = "color: #e6eff7; font-weight: 700;"
        lbl_scale.setStyleSheet(label_style)

        def _sync_scale_label(v: int):
            lbl_scale_val.setText(f"{v}%")

        slider_scale.valueChanged.connect(_sync_scale_label)
        _sync_scale_label(slider_scale.value())

        # Default directories
        ed_maps = QLineEdit(str(self.default_maps_dir))
        ed_videos = QLineEdit(str(self.default_videos_dir))
        ed_tokens = QLineEdit(str(self.default_tokens_dir))
        ed_sessions = QLineEdit(str(self.default_sessions_dir))

        def _pick_dir(target: QLineEdit):
            base = target.text().strip() or str(self._fallback_browser_dir())
            chosen = QFileDialog.getExistingDirectory(dlg, "Select folder", base)
            if chosen:
                target.setText(str(Path(chosen).resolve()))

        btn_maps = QPushButton("Browse…")
        btn_videos = QPushButton("Browse…")
        btn_tokens = QPushButton("Browse…")
        btn_sessions = QPushButton("Browse…")
        btn_maps.clicked.connect(lambda: _pick_dir(ed_maps))
        btn_videos.clicked.connect(lambda: _pick_dir(ed_videos))
        btn_tokens.clicked.connect(lambda: _pick_dir(ed_tokens))
        btn_sessions.clicked.connect(lambda: _pick_dir(ed_sessions))

        # Grid default behavior
        cmb_grid_default = QComboBox()
        cmb_grid_default.addItems(["None", "Square", "Hex"])
        idx = cmb_grid_default.findText(self.grid_default_on_enable)
        cmb_grid_default.setCurrentIndex(idx if idx >= 0 else 1)

        lbl_maps = QLabel("Default maps folder")
        lbl_videos = QLabel("Default videos folder")
        lbl_tokens = QLabel("Default tokens folder")
        lbl_sessions = QLabel("Default sessions folder")
        lbl_grid_default = QLabel("Grid default when enabling Grid")
        for _lbl in (lbl_maps, lbl_videos, lbl_tokens, lbl_sessions, lbl_grid_default):
            _lbl.setStyleSheet(label_style)

        grid.addWidget(lbl_scale, 0, 0)
        grid.addWidget(slider_scale, 0, 1)
        grid.addWidget(lbl_scale_val, 0, 2)

        grid.addWidget(lbl_maps, 1, 0)
        grid.addWidget(ed_maps, 1, 1)
        grid.addWidget(btn_maps, 1, 2)

        grid.addWidget(lbl_videos, 2, 0)
        grid.addWidget(ed_videos, 2, 1)
        grid.addWidget(btn_videos, 2, 2)

        grid.addWidget(lbl_tokens, 3, 0)
        grid.addWidget(ed_tokens, 3, 1)
        grid.addWidget(btn_tokens, 3, 2)

        grid.addWidget(lbl_sessions, 4, 0)
        grid.addWidget(ed_sessions, 4, 1)
        grid.addWidget(btn_sessions, 4, 2)

        grid.addWidget(lbl_grid_default, 5, 0)
        grid.addWidget(cmb_grid_default, 5, 1)

        lay.addLayout(grid)

        hint = QLabel("These parameters are saved in data/settings.ini and applied immediately.")
        hint.setStyleSheet("color: #d7e6f2;")
        lay.addWidget(hint)
        ...
        btn_save.setProperty("vfRole", "primary")
        ...
        def _save_params():
            fallback = self._fallback_browser_dir()
            self.default_maps_dir = self._sanitize_existing_dir(ed_maps.text().strip(), fallback)
            self.default_videos_dir = self._sanitize_existing_dir(ed_videos.text().strip(), self.default_maps_dir)
            self.default_tokens_dir = self._sanitize_existing_dir(ed_tokens.text().strip(), self.default_maps_dir)
            self.default_sessions_dir = self._sanitize_or_create_dir(ed_sessions.text().strip(), self._app_dir() / "data" / "sessions")

            self.token_import_scale = max(0.4, min(3.0, float(slider_scale.value()) / 100.0))
            self.canvas.token_import_scale = float(self.token_import_scale)

            self.grid_default_on_enable = cmb_grid_default.currentText()

            self.settings.setValue("default_maps_dir", str(self.default_maps_dir))
            self.settings.setValue("default_videos_dir", str(self.default_videos_dir))
            self.settings.setValue("default_tokens_dir", str(self.default_tokens_dir))
            self.settings.setValue("default_sessions_dir", str(self.default_sessions_dir))
            self.settings.setValue("token_import_scale", float(self.token_import_scale))
            self.settings.setValue("grid_default_on_enable", self.grid_default_on_enable)

            self.statusBar().showMessage("Parameters saved", 1800)
            dlg.accept()

        btn_save.clicked.connect(_save_params)
        dlg.exec()
    ...
    def on_grid_changed(self, *_args):
        ...
        # If Grid is enabled and the type is still 'None', use configurable default.
        if show and (not gtype or gtype == "None"):
            default_type = self.grid_default_on_enable if self.grid_default_on_enable in ("Square", "Hex") else "Square"
            idx = self.cmb_grid.findText(default_type)
            if idx >= 0:
    ...
    def import_token(self):
        ...
        """
        has_canvas_map = getattr(self.canvas, "_map", None) is not None
        if not self.loaded or (not self.map_img and not has_canvas_map):
            QMessageBox.information(self, "Import Token", "Load a map first (for videos, wait for first frame).")
            return
        paths, _ = QFileDialog.getOpenFileNames(
            self,
            "Import Token(s)",
            str(self.default_tokens_dir),
            "Token images (*.png *.jpg *.jpeg);;All files (*.*)",
        )
        if not paths:
            return

        map_img = getattr(self.canvas, "_map", None)
        if map_img is None:
            QMessageBox.information(self, "Import Token", "Load a map first (for videos, wait for first frame).")
            return

        mw = float(map_img.width())
        mh = float(map_img.height())
        base_cx = mw / 2.0
        base_cy = mh / 2.0
        base_spacing = max(40.0, float(getattr(self, "token_batch_spacing", 96.0)))
        if self.chk_grid.isChecked() and self.cmb_grid.currentText() in ("Square", "Hex"):
            step = max(base_spacing, float(self.spin_grid.value()) * 1.1)
        else:
            step = base_spacing
        cols = 5
        start_idx = len(getattr(self.canvas, "tokens", []) or [])
        ...
        for local_idx, path in enumerate(paths):
            ...
            slot = start_idx + local_idx
            col = slot % cols
            row = slot // cols
            dx = (float(col) - (cols - 1) / 2.0) * step
            dy = float(row) * step
            pos = QPointF(base_cx + dx, base_cy + dy)
            self.canvas.add_token_image(img, pos)
            imported += 1

        if imported <= 0:
            QMessageBox.warning(self, "Import Token", "No valid image selected.")
            return

        if invalid > 0:
            self.statusBar().showMessage(f"{imported} token(s) imported, {invalid} invalid file(s) skipped", 2800)
        else:
            self.statusBar().showMessage(f"{imported} token(s) imported", 2200)
    ...
    def calibrate_display_dialog(self):
        ...
        def save():
            ...
            if hasattr(self, "btn_calib_display"):
                self.btn_calib_display.setToolTip(f'Current calibration: ppi : {self.px_per_inch_real:.2f}')
            dlg.accept()
            self.update_player_view()
...
# ---------- Map ----------
    ...
    def open_map(self):
        ...
        if not self._maybe_prompt_save_before_discard():
            return
        start_dir = str(self.default_videos_dir if self.last_open_media_kind == "video" else self.default_maps_dir)
        path, _ = QFileDialog.getOpenFileName(
            self, "Open Map", start_dir, "Maps/Sessions (*.png *.jpg *.jpeg *.tif *.tiff *.pdf *.json *.mp4 *.webm *.m4a *.mkv);;All files (*.*)"
        )
        if not path:
            return
        ...
        if not Path(path).exists():
            ...
            return
        self.last_open_media_kind = "video" if self._is_video_path(path) else "map"
        self.settings.setValue("last_open_media_kind", self.last_open_media_kind)
    ...
    def _do_save_to(self, session_path: Path) -> bool:
        if not self.loaded or self.canvas.mask_img is None:
